...
Shader "NoeNoe/NoeNoe Overlay Shader/Misc/NoeNoe Overlay Advanced" {
    Properties {
        ...
        _OverlayMap ("Overlay Map", 2D) = "gray" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                clip((SurfaceAlpha*OverlayAlpha) - 0.5);
...
////// Emissive:
                ...
                float node_3700 = 1.0;
                float OverlayCrossfade = lerp(0,2,(((((clamp(_CrossfadeTileCubemap,0,1)*_OverlayMap_var.b)-(((-1*(clamp(_CrossfadeTileCubemap,1,2)-node_3700))+node_3700)*_OverlayMap_var.r))*2.0+0.0)+_CrossfadeTileCubemap)*0.5+0.0));
                float node_640 = OverlayCrossfade;
                float3 Overlay = (lerp(float3(node_6078,node_6078,node_6078),(texCUBE(_CubemapOverlay,RotatedCubemapDirection).rgb*2.0),saturate(node_640))*lerp(float3(node_6078,node_6078,node_6078),_TileOverlay_var.rgb,saturate((node_6078+(1.0 - node_640)))));
                float4 node_1973_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_1973_p = lerp(float4(float4(_OverlayMap_var.rgb,0.0).zy, node_1973_k.wz), float4(float4(_OverlayMap_var.rgb,0.0).yz, node_1973_k.xy), step(float4(_OverlayMap_var.rgb,0.0).z, float4(_OverlayMap_var.rgb,0.0).y));
                float4 node_1973_q = lerp(float4(node_1973_p.xyw, float4(_OverlayMap_var.rgb,0.0).x), float4(float4(_OverlayMap_var.rgb,0.0).x, node_1973_p.yzx), step(node_1973_p.x, float4(_OverlayMap_var.rgb,0.0).x));
                float node_1973_d = node_1973_q.x - min(node_1973_q.w, node_1973_q.y);
                float node_1973_e = 1.0e-10;
                float3 node_1973 = float3(abs(node_1973_q.z + (node_1973_q.w - node_1973_q.y) / (6.0 * node_1973_d + node_1973_e)), node_1973_d / (node_1973_q.x + node_1973_e), node_1973_q.x);;
                float OverlayMapValue = node_1973.b;
                float node_8875 = lerp(0,2,(((OverlayMapValue*2.0+-1.0)+_CrossfadeSurfaceOverlay)*saturate(_CrossfadeSurfaceOverlay)));
                float OverlayFactor = saturate(node_8875);
                float SurfaceFactor = saturate((1.0+(1.0 - node_8875)));
                ...
                float node_8152 = 0.5*dot(node_2136,lightDirection)+0.5;
...
////// Lighting:
                ...
                float4 node_3501 = tex2D(_Ramp,TRANSFORM_TEX(node_795, _Ramp));
                float3 DynamicToonLighting = lerp((node_5356_if_leA*node_3734)+(node_5356_if_leB*node_3734),node_3501.rgb,node_5356_if_leA*node_5356_if_leB);
                float3 finalColor = saturate(((_Intensity*FlatLighting*Diffuse) > 0.5 ?  (1.0-(1.0-2.0*((_Intensity*FlatLighting*Diffuse)-0.5))*(1.0-lerp(float3(node_424,node_424,node_424),(StaticToonLighting*DynamicToonLighting),_ToonContrast))) : (2.0*(_Intensity*FlatLighting*Diffuse)*lerp(float3(node_424,node_424,node_424),(StaticToonLighting*DynamicToonLighting),_ToonContrast))) );
